  ...
  render() {
    ...
    return (
      <article className="item-prod" id="article-template">
        ...
        <span className="precio-prod">Precio: {this.state.precio}</span>
        <br />
        <span className="stock-prod">Stock: {this.state.stock}</span>
        <br />
        <span>
          <b>- Categoria: </b> {this.state.nameCat}
        </span>

        {/* 🟧 BOTÓN PARA BORRAR */}
        <button
          className="btn-eliminar"
          onClick={() => this.props.onDelete(this.props.prodCarrito.id)}
        >
          Eliminar ❌
        </button>
      </article>
